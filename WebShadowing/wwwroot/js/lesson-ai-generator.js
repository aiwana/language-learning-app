...
(function () {
    ...
    async function loadLibrary() {
        ...
        if (!list) return;
        try {
            const [draftResponse, savedResponse] = await Promise.all([
                fetch("/api/ai-lessons/previews", { cache: "no-store" }),
                fetch("/api/ai-lessons", { cache: "no-store" })
            ]);
            if (!draftResponse.ok || !savedResponse.ok) throw new Error("Không tải được bài AI.");
            const drafts = await draftResponse.json();
            const saved = await savedResponse.json();
            const items = [
                ...drafts.map(item => ({ ...item, kind: "draft" })),
                ...saved.map(item => ({ ...item, kind: "saved" }))
            ];
