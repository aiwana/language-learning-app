...
(function () {
    ...
    async function submitAnswer(tab, sentence, answer, resultId, onDone) {
        const incorrectMessage = tab === "ipa-match"
            ? "Chưa đúng. Hãy thử lại."
            : "Chưa đúng. Nghe lại và thử thêm một lần.";
        if (lesson.source === "ai-generated") {
            ...
            setText(resultId, passed ? "Đúng. Bạn đã mở câu tiếp theo." : incorrectMessage);
            onDone(passed);
            return;
        }
        ...
        try {
            ...
            setText(resultId, result.passed ? "Đúng. Bạn đã mở câu tiếp theo." : incorrectMessage);
